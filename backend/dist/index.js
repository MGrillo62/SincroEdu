...
app.get('/api/tenants/:tenantId/roles', authenticateJWT, (req, res) => {
    ...
    const tenantRoles = db_1.roles.filter(r => r.tenantId === tenantId ||
...
app.get('/api/tenants/:tenantId/users', authenticateJWT, async (req, res) => {
    const { tenantId } = req.params;
    // Aislamiento Multi-Tenant estricto
    if (req.user?.tenantId && req.user.tenantId !== tenantId) {
        return res.status(403).json({ error: 'Acceso denegado: Aislamiento Tenant violado' });
    ...
    if (req.user?.roleId !== 'r-superadmin' && req.user?.roleId !== 'r-tenant1-admin') {
        const isCustomAdmin = db_1.roles.find(r => r.id === req.user?.roleId && r.tenantId === tenantId && r.name.toLowerCase() === 'admin');
        if (!isCustomAdmin) {
    ...
    if (dbAvailable) {
      ...
      `;
        ...
        // Fallback en memoria
        const mapped = db_1.users.filter(u => u.tenantId === tenantId).map(u => {
            const role = db_1.roles.find(r => r.id === u.roleId);
            return {
                id: u.id,
                tenantId: u.tenantId,
                roleId: u.roleId,
...
app.post('/api/tenants/:tenantId/users', authenticateJWT, async (req, res) => {
    const { tenantId } = req.params;
    ...
    try {
        ...
        const passHash = bcryptjs_1.default.hashSync(password || 'sincro123', salt);
      ...
      `;
            ...
            // Validar en memoria
            const emailExists = db_1.users.some(u => u.email.toLowerCase() === email.toLowerCase());
            if (emailExists) {
                return res.status(409).json({ error: 'El correo electrónico ya se encuentra registrado' });
            }
            const newUser = {
...
app.put('/api/tenants/:tenantId/users/:userId', authenticateJWT, async (req, res) => {
    ...
    try {
      ...
      `;
            ...
            const idx = db_1.users.findIndex(u => u.id === userId && u.tenantId === tenantId);
            if (idx === -1) {
                return res.status(404).json({ error: 'Usuario no encontrado' });
            }
            db_1.users[idx].firstName = firstName || db_1.users[idx].firstName;
            db_1.users[idx].lastName = lastName || db_1.users[idx].lastName;
            db_1.users[idx].phone = phone !== undefined ? phone : db_1.users[idx].phone;
            db_1.users[idx].roleId = roleId || db_1.users[idx].roleId;
            db_1.users[idx].isActive = isActive !== undefined ? !!isActive : db_1.users[idx].isActive;
...
app.delete('/api/tenants/:tenantId/users/:userId', authenticateJWT, async (req, res) => {
    const { tenantId, userId } = req.params;
    // Aislamiento Multi-Tenant
    if (req.user?.tenantId && req.user.tenantId !== tenantId) {
        return res.status(403).json({ error: 'Acceso denegado: Aislamiento Tenant violado' });
